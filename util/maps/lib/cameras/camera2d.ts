...
export class camera2d {
  ...
  #handleMove(x: number, y: number) {
    ...
    if (this._mouseDown) {
      if (!this._isDragging) {
        ...
        const p = this._lastDrag.clone().subtract(newPos.clone()).divide(new Vector2(this.Zoom, this.Zoom))
        this.Position.x += p.x
        this.Position.y += p.y
      }
  ...
  ScreenToWorld(screenPosition: Vector3) {
    return screenPosition.clone().transform(this.TranslationMatrix.clone().invert())
  }
  ...
  ScreenToCoords(screenPosition: Vector3) {
    ...
    const offset = (new Vector3(worldPos.x, this.MapSize.y - worldPos.y))

    if (offset.x < 0) offset.x = 0
    if (offset.y < 0) offset.y = 0

    const landblock = (((Math.min(Math.floor(offset.x / 192.0), 0xFE)) * Math.pow(2, 24)) + (Math.min(Math.floor(offset.y / 192.0), 0xFE) << 16))
