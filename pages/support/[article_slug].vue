<template>
  <PageArticle>
    ...
    <div class="flex-1">
      ...
      <ContentDoc
        :path="'support/' + route.params.article_slug"
...
const { data } = await useAsyncData(
  "support-article-" + route.params.article_slug,
  () => queryContent("support/" + route.params.article_slug).findOne()
);
...
onMounted(async () => {
  const surround = await queryContent()
    .only(["_path", "title"])
    .sort({ date: -1 })
    .findSurround("/support/" + route.params.article_slug);
